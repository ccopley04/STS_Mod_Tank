...
public class DefensiveInvestment_Tank extends BaseCard{

    public static final String ID = makeID("Defensive Investment");

    public static final int INVESTMENT = 2;
    public static final int UPG_INVESTMENT = 1;
    ...
    public DefensiveInvestment_Tank(){
        super(ID, info);

        setMagic(INVESTMENT, UPG_INVESTMENT);
    }

    public void use(AbstractPlayer p, AbstractMonster m){
        addToBot(new ApplyPowerAction(p, p, new ExposedPower(p, 2)));
        addToBot(new ApplyPowerAction(p,p, new InvestmentPower(p, this.magicNumber)));
    }
}
